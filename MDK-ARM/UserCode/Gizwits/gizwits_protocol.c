...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagElectricity)
    {
        info->event[info->num] = EVENT_Electricity;
        info->num++;
        dataPoints->valueElectricity = gizStandardDecompressionValue(Electricity_BYTEOFFSET,Electricity_BITOFFSET,Electricity_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueElectricity != cur->valueElectricity)
    {
        GIZWITS_LOG("valueElectricity Changed\n");
        ret = 1;
    }
    ...
    if(last->valueAirWarn != cur->valueAirWarn)
    {
        GIZWITS_LOG("valueAirWarn Changed\n");
        ret = 1;
    }
    if(last->valueFire != cur->valueFire)
    {
        GIZWITS_LOG("valueFire Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Electricity_BYTEOFFSET,Electricity_BITOFFSET,Electricity_LEN,(uint8_t *)devStatusPtr,dataPoints->valueElectricity);
    if(dataPoints->valueFan >= Fan_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueFan Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
    ...
    gizStandardCompressValue(AirWarn_BYTEOFFSET,AirWarn_BITOFFSET,AirWarn_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAirWarn);
    gizStandardCompressValue(Fire_BYTEOFFSET,Fire_BITOFFSET,Fire_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFire);
